...
CGLM_INLINE
vec3s
glms_unprojecti_zo(vec3s pos, mat4s invMat, vec4s vp, vec3s dest) {
  vec3s dest;
...
CGLM_INLINE
vec3s
glms_project_zo(vec3s pos, mat4s m, vec4s vp, vec3s dest) {
  vec3s dest;
...
glms_project_z_zo(vec3s v, mat4s m) {
  // FIXME: Alterei o tipo de retorno de vec3s para float
